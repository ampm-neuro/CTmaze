function [all_aysm_scores, all_zscore_sections] = allztrial
%finding zscore differences for each section for each session
%
...
subject_combined_aysm_scores = [];
subject_combined_zscore_sections = [];
all_aysm_scores = [];
all_zscore_sections = [];
...
for subject = 1:length_subjects
    ...
    for stage = 1:length_stages
        ...
        for session = 1:length_sessions
            ...
            [aysm_scores, zscore_sections] = ztrial(eptrials, clusters);
            %combining the within-rat outputs
            aysm_scores = [subject.*ones(length(aysm_scores(:,1)), 1) stage.*ones(length(aysm_scores(:,1)), 1) session.*ones(length(aysm_scores(:,1)), 1) aysm_scores];
            zscore_sections = [subject.*ones(length(zscore_sections(:,1)), 1) stage.*ones(length(zscore_sections(:,1)), 1) session.*ones(length(zscore_sections(:,1)), 1) zscore_sections];
            
            
            subject_combined_aysm_scores = [subject_combined_aysm_scores; aysm_scores];
     
            subject_combined_zscore_sections = [subject_combined_zscore_sections; zscore_sections];
            
        end
    end
    %combining across rats
    all_aysm_scores = [all_aysm_scores; subject_combined_aysm_scores];
    all_zscore_sections = [all_zscore_sections; subject_combined_zscore_sections];
