    ...
    for stage = 2%1:length_stages
        ...
        for session = 1:length_sessions
            ...
            if length(clusters)>1%screens empty cluster files to avoid errors
                ...
                %count = [count length(clusters(clusters(:,2)>2,1))];d
                                
                [folded_section_area, ~] = visit_distribution(eptrials);
                
                area = [area folded_section_area./sum(folded_section_area)];
