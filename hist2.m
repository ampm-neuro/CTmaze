function hist2(eptrials, c, bins)
...
xs = eptrials(eptrials(:,4)==c & eptrials(:,8)==1, 2);
ys = eptrials(eptrials(:,4)==c & eptrials(:,8)==1, 3);

%all time samples in two vectors 
xt = eptrials(eptrials(:,14)==1 & eptrials(:,8)==1, 2);
yt = eptrials(eptrials(:,14)==1 & eptrials(:,8)==1, 3);
...
%APPROPRIATE FOR NON-TRIAL BASED DATA (CAN BE COMMENTED OUT).
    ...
    multi_trial_pixles = pixle_per_trial(ismember(n, find(hist_counts>3)), :); %pixles visited on >1 trials
...
%hist2trials(eptrials, c, bins, b, d, mask, size_fix)