function [classification_success, posterior_all, class_all, class_shuf, group_ID, p_sections_norm, trial_type_idx, p_sections_NotNormed, p_sections_numpix] = rev_decodehist_ShufCellID(eptrials, cells, bins, window, slide, vect2mat_idx, shuffs, varargin) 
...
for trial = trials
    ...
    % normalize spike counts (Dont do this)
    %{
    norm_counts = [training; sample];
    norm_counts = norm_counts - min(norm_counts);
    norm_counts = norm_counts./max(norm_counts);
    training = norm_counts(1:size(training,1), :);
    sample = norm_counts(size(training,1)+1 : end, :);
    %}
    
    
    % SHUFFLE SAMPLE COLUMNS (shuffle cell ID)
    %
    %
    %
    %
    ...
    %figure; plot(nansum(posterior)); title(num2str(nansum(nansum(posterior))))
