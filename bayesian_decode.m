...
%           cell spike counts for columns.
%       'group' is a vector of pixel ids describing the rat was at each 
...
%
    ...
    %
        ...
        ntrain = size(training,1); %used later (maybe)
...
%
    ...
    %
    % This is the probability for spike counts B to occur given that 
    ...
    %
    % Where C(tao,B) is a normalazation factor, such that sum(P(A|B)) is equal to 1.
    %
    % This gives us a matrix sample_timesamples*pixles
        ...
        %
        for icell = 1:size(training,2)
            figure; plot(f_x(:,icell)*tao); title('shuf')
            hold on; plot(xlim, o_spikes(:,icell).*[1 1] )
        end
    
        figure; plot(P_A_B(tsamp, :))
        %}
