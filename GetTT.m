...
if a>0
    ...
    for i = 1:length(b)
        ...
        d = [TimestampsTT', zeros(length(TimestampsTT), 1), zeros(length(TimestampsTT), 1), CellNumbers', zeros(length(TimestampsTT), 1), NaN(length(TimestampsTT), 1), NaN(length(TimestampsTT), 1), zeros(length(TimestampsTT), 1)];
    
        %creates possibly unneccessary column with TTs of orgin
        d(:,6) = c(i);
