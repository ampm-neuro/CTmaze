function rwdpredict_UW(eptrials, cells)
...
RWD_REP_distance = ((nanmean(LRR)-nanmean(RRR))*(inv(COV))*(nanmean(LRR)-nanmean(RRR))')/length(cells(clusts == 0))
...
C_L_rates = squeeze(rwdwindowrates(pleft & pcorrect, 1, :)); %RATES AT RWD BEFORE L-SUCCESS (RIGHT RWD)
C_R_rates = squeeze(rwdwindowrates(pright & pcorrect, 1, :)); %RATES AT RWD BEFORE R-SUCCESS (LEFT RWD)
...
E_R_rates = reshape(rwdwindowrates(pright & perror, 1, :, :), length(rwdwindowrates(pright & perror, 1, 1, 1)), length(cells)); %RATES BEFORE L-ERROR (LEFT RWD)
