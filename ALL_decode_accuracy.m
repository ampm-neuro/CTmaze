function [time_class, space_class, prop_pages, pp_idx, class_all_comb, time_class_sesh_shuf, space_class_shuf, prop_pages_shuf, class_shuf_comb, space_class_d, space_class_smooth, space_class_smooth_d, p_sections, accuracy, posterior_all_cell, sessions_cell, sesh_vel_pos_comb, time_class_sesh, trial_type_idx_comb, group_ID_comb, p_sections_nnorm, p_sections_area] = ALL_decode_accuracy(learning_stages, bins, window, slide, page_overlap, vect2mat_idx, shuffs)
...
for subject = 1:length_subjects
    ...
    for stage = floor(learning_stages)
        ...
        %if learning_stages==2.3
        %    lastsesh = file_list_sessions(end).name;
        %end
        
        %learning_stages
        %file_list_sessions.name
        ...
        for session = sesh_rng
            ...
            if length(clusters(:,1))>=8
                ...
                %classify 
                %stems
                %[classification_success, posterior_all, class_all, class_shuf, group_ID, p_sections_norm, trial_type_idx, p_sections_NoNorm, p_sections_numpix] = decodehist(eptrials, clusts, bins, window, slide, vect2mat_idx, shuffs, stem_runs);
                %all
                [classification_success, posterior_all, class_all, class_shuf, group_ID, p_sections_norm, trial_type_idx, p_sections_NoNorm, p_sections_numpix] = decodehist(eptrials, clusts, bins, window, slide, vect2mat_idx, shuffs);
                %rewards
    ...
    for i = unique(class_all_comb(:,5))'
        ...
        acc_decs = sum(time_class_dists(~isnan(time_class_dists) & class_all_comb(:,5)==i)<radius)
        num_decs = length(time_class_dists(~isnan(time_class_dists) & class_all_comb(:,5)==i))
        prop_acc_decs = acc_decs/num_decs
       
        time_class_sesh(i) = prop_acc_decs;
    end
    ...
    for shf = 1:shuffs
        ...
        for i = unique(class_all_comb(:,5))'
            ...
            acc_decs = sum(time_class_dists_shuf(~isnan(time_class_dists_shuf) & class_all_comb(:,5)==i)<radius);
            num_decs = length(time_class_dists_shuf(~isnan(time_class_dists_shuf) & class_all_comb(:,5)==i));
            prop_acc_decs = acc_decs/num_decs;
            
            time_class_sesh_shuf(i, shf) = prop_acc_decs;
        end
    end

    %smoke figure
    %[space_class, space_class_d, space_class_smooth, space_class_smooth_d] = smoke_fig(prop_pages, page_overlap, 1);%floor(size(prop_pages,3)\2))
    
    %space shuffle
    %{
    space_class_shuf = nan(shuffs, 1);
    for stack = 1:shuffs
        space_class_shuf(stack) = smoke_fig(prop_pages_shuf(:, :, :, stack), page_overlap, 0);
    end
    %}
end
