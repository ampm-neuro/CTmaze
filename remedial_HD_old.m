...
difference_vector = smooth(difference_vector, 20)'; %this should not be circ_smoothed
...
for sample = 1:size(candidate_hds, 2)
    ...
    distances = [circ_distance(trajectory_angle(sample), candidate_hds(1, sample), [0 360]);
   	...
   	%head direction is the closest candidate hd
...
head_direction = circ_smooth(head_direction, [0 360], 20);
