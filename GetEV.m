...
[TimeStamps, EventIDs, TTLs, Extras, EventStrings, Header] = Nlx2MatEV_v3(filename, [1 1 1 1 1], 1, 1, []);

%places a 1 in rows with a timestamp (fifth column)
flags = [TimeStamps', zeros(length(TimeStamps), 1), zeros(length(TimeStamps), 1), NaN(length(TimeStamps), 1), ones(length(TimeStamps), 1), NaN(length(TimeStamps), 1), NaN(length(TimeStamps), 1), zeros(length(TimeStamps), 1)];
